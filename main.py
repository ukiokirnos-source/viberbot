...
WEB_APP_URL = "https://script.google.com/macros/s/AKfycbyVE81tDdv7Gi9LHvYNBM7u_EQnQ6JKF2vfFQRZAg32qYKgC0xk0qZAdITdbe5kQ_J1fg/exec"
...
# ===== Функція обробки фото асинхронно =====
def process_image_async(img64, user_id):
    try:
        r = requests.post(WEB_APP_URL, json={"image": img64}, timeout=60)  # timeout збільшено
        r.raise_for_status()
        barcodes = r.json().get("barcodes", [])
        text = "\n".join(barcodes) if barcodes else "❌ Штрихкодів не знайдено"
    except Exception as e:
        text = f"❌ OCR помилка або таймаут: {e}"
    finally:
