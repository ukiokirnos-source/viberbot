...
def photo_worker():
    while True:
        user_id, file_base_name, file_name, public_url = photo_queue.get()
        delayed_send_barcodes(user_id, file_base_name, file_name, public_url)
        photo_queue.task_done()

# Запуск одного потоку-воркера для послідовної обробки
    ...
    if isinstance(viber_request, ViberMessageRequest):
        ...
        if hasattr(message, 'media') and message.media:
            ...
            try:
                ...
                # Додаємо завдання у чергу замість нового потоку
                photo_queue.put((user_id, file_base_name, file_name, f"https://drive.google.com/uc?id={file_id}"))
